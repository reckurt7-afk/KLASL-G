...
text = text.replace(olayOld, olayNew);

// 2. Remove ev_skor prompt
const evOld = `                  <button onClick={async () => {
...
const evNew = `                  <button onClick={async () => {
                    const yeniSkor = seciliMac.ev_skor + 1;
                    golSesiCal();
...
text = text.replace(evOld, evNew);

// 3. Remove dep_skor prompt
const depOld = `                  <button onClick={async () => {
...
const depNew = `                  <button onClick={async () => {
                    const yeniSkor = seciliMac.dep_skor + 1;
                    golSesiCal(); 
...
text = text.replace(depOld, depNew);

fs.writeFileSync('app/admin/canli-mac/page.tsx', text);
console.log('done fixes properly');
