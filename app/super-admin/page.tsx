      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center">
            <span className="text-4xl font-black text-[#ceaa52] mb-2">{cities.length}</span>
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <button 
            ...
            className="bg-[#ceaa52] hover:bg-[#9e1b22] text-white px-5 py-2 rounded-lg font-bold transition-colors"
          >
            + Yeni Şehir Ekle
          </button>
        </div>
            ...
            <div key={city.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
              <div className="h-24 bg-gradient-to-r from-gray-800 to-gray-900 relative">
                {city.status === 'AKTIF' ? (
                  ...
                  <span className="absolute top-3 right-3 bg-[#eab308] text-white text-[10px] font-bold px-2 py-1 rounded-full">PASİF</span>
                )}
              </div>
              <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="mt-auto pt-4 border-t border-gray-100 flex justify-between items-center">
                  ...
                  <button onClick={() => setManageCity(city)} className="text-[#ceaa52] hover:text-[#9e1b22] text-sm font-bold">
                    Yönet -&gt;
                  </button>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl relative">
            ...
            <form onSubmit={handleCreateCity} className="space-y-4">
              <div>
                ...
                <input type="text" value={newCity.name} onChange={(e) => setNewCity({...newCity, name: e.target.value})} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#9e1b22] focus:border-transparent outline-none" placeholder="PRİME LİG İSTANBUL" required />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Slug (URL için, Örn: istanbul)</label>
                <input type="text" value={newCity.slug} onChange={(e) => setNewCity({...newCity, slug: e.target.value})} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#9e1b22] focus:border-transparent outline-none" placeholder="istanbul" required />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Plaka / Şehir Kodu (Örn: 34)</label>
                <input type="text" value={newCity.code} onChange={(e) => setNewCity({...newCity, code: e.target.value})} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#9e1b22] focus:border-transparent outline-none" placeholder="34" required />
              </div>
              <button type="submit" disabled={isSubmitting} className="w-full bg-[#ceaa52] hover:bg-[#9e1b22] text-white font-bold py-3 rounded-lg transition-colors mt-4">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-6">
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-1 border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-[#9e1b22] focus:border-transparent outline-none"
                />
                <button 
                  onClick={async () => {
                  ...
                  className="bg-[#ceaa52] hover:bg-[#9e1b22] text-white font-bold px-4 py-2 rounded-lg transition-colors"
                >
                  Yetki Ver
                </button>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                <button 
                  ...
                  className="text-[#eab308] hover:text-[#b5952f] hover:bg-red-50 font-bold px-4 py-2 rounded-lg transition-colors flex items-center gap-1"
                >
