...
export default function FiksturPage() {
  ...
  function getStatusBadge(mac: Mac) {
    if (mac.canli) return { label: "CANLI", cls: "bg-[#ceaa52] text-white animate-pulse" };
  ...
  return (
    <div className="w-full flex flex-col font-sans fade-in">
      ...
      {loading ? (
        <div className="w-full py-20 flex justify-center"><div className="w-8 h-8 border-4 border-[#eab308] border-t-transparent rounded-full animate-spin"></div></div>
      ) : haftalar.length === 0 ? (
            ...
            {haftalar.map((h) => (
              <button
                ...
                className={`shrink-0 px-4 py-2 rounded-full font-black text-[14px] transition-all ${
                  aktifHafta === h.hafta
                    ? "bg-[#ceaa52] text-white shadow-md scale-105"
                ...
                return (
                  <div
                    ...
                    className={`bg-white border rounded-2xl overflow-hidden shadow-[0_4px_12px_rgba(0,0,0,0.03)] hover:shadow-md transition-all ${
                      mac.canli ? "border-[#ceaa52]/40 ring-1 ring-[#9e1b22]/20" : "border-gray-100"
                    }`}
                  >
                    {/* Top Bar */}
                      ...
                      <div className="flex flex-col items-center justify-center px-2 shrink-0">
                        {mac.oynandi || mac.canli ? (
                          <div className="flex items-center gap-3">
                            <span className={`text-[36px] font-black tracking-tighter leading-none ${mac.canli ? "text-[#ceaa52] animate-pulse" : "text-[#1a1a2e]"}`}>
                              {mac.ev_skor ?? 0}
                            </span>
                          ...
                          <div className="flex flex-col items-center gap-1.5 w-full max-w-[140px]">
                            {mac.saha && (
                              <span className="text-[10px] text-[#ceaa52] font-black uppercase tracking-wider text-center bg-red-50 px-2 py-0.5 rounded w-full line-clamp-1">
                                {mac.saha}
                              </span>
                            )}
