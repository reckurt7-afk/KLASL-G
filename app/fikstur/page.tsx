...
export default function FiksturPage() {
  ...
  function getStatusBadge(mac: Mac) {
    if (mac.canli) return { label: "CANLI", cls: "bg-[#1e3a8a] text-white animate-pulse" };
                ...
                return (
                  <div
                    ...
                    className={`bg-white border rounded-2xl overflow-hidden shadow-[0_4px_12px_rgba(0,0,0,0.03)] hover:shadow-md transition-all ${
                      mac.canli ? "border-[#1e3a8a]/40 ring-1 ring-[#1e3a8a]/20" : "border-gray-100"
                    }`}
                  >
                    {/* Top Bar */}
                      ...
                      <div className="flex flex-col items-center justify-center px-2 shrink-0">
                        {mac.oynandi || mac.canli ? (
                          <div className="flex items-center gap-3">
                            <span className={`text-[36px] font-black tracking-tighter leading-none ${mac.canli ? "text-[#1e3a8a] animate-pulse" : "text-[#1a1a2e]"}`}>
                              {mac.ev_skor ?? 0}
                            </span>
                          ...
                          <div className="flex flex-col items-center gap-1.5 w-full max-w-[140px]">
                            {mac.saha && (
                              <span className="text-[10px] text-[#1e3a8a] font-black uppercase tracking-wider text-center bg-red-50 px-2 py-0.5 rounded w-full line-clamp-1">
                                {mac.saha}
                              </span>
                            )}
