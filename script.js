...
if (!code.includes('useState')) {
  code = code.replace('import Image', 'import { useState, useRef } from \'react\';\nimport Image');
}

code = code.replace('export default function Splash() {', 'export default function Splash() {\n  const [isMuted, setIsMuted] = useState(true);\n  const videoRef = useRef(null);');

const oldVideoRegex = /\{\/\* Background MP4 Video \*\/\}[\s\S]*?<div className="absolute inset-0 z-10 bg-black\/40 pointer-events-none"><\/div>/;
...
const newVideo = 
          ...
          <button 
            ...
            className="absolute bottom-8 right-8 z-[100] bg-black/50 hover:bg-black/80 text-white p-3 rounded-full backdrop-blur-md transition-all shadow-[0_0_20px_rgba(0,0,0,0.5)] border border-white/20"
...
code = code.replace(oldVideoRegex, newVideo);
