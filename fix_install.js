...
export default function InstallButton() {
  ...
  const installApp = async () => {
    ...
    if (!installPrompt) {
      alert("Cihazýnýzda otomatik yükleme desteklenmiyor. Tarayýcý menüsünden 'Ana Ekrana Ekle' (Add to Home Screen) seçeneðini kullanabilirsiniz.");
      return;
    }
    ...
    try {
      ...
      console.error("Yükleme hatasý:", err);
    }
  ...
  return (
    <div className="relative w-full max-w-[340px] mx-auto flex flex-col items-center">
      <button
        onClick={installApp}
        disabled={loading}
        className="group relative overflow-hidden bg-[#e60000] hover:bg-[#cc0000] text-white rounded-xl px-6 h-[56px] flex items-center justify-center gap-3 w-full font-black text-[15px] shadow-[0_4px_20px_rgba(230,0,0,0.4)] hover:shadow-[0_6px_25px_rgba(230,0,0,0.5)] transition-all hover:-translate-y-0.5"
      >
      ...
      {showIOSPrompt && (
        <div className="absolute -bottom-[80px] left-1/2 -translate-x-1/2 w-[280px] bg-[#1a1a2e] text-white text-[12px] p-3 rounded-lg shadow-xl border border-white/10 text-center z-50 animate-fade-in">
          ...
          <span className="font-bold text-[#e60000]">"Paylaþ"</span> ikonuna týklayýp <br/>
          <span className="font-bold text-[#e60000]">"Ana Ekrana Ekle"</span> seçeneðini kullanýn.
...
;
fs.writeFileSync('app/components/InstallButton.tsx', content, 'utf8');
